...
async fn validate(src_consensus: &Consensus, dst_consensus: &Consensus, params: &Params, delay: f64, bps: f64) {
    ...
    info!("Validating {} blocks with {} transactions overall...", num_blocks, num_txs);
...
fn print_stats(src_consensus: &Consensus, hashes: &[Hash], delay: f64, bps: f64, k: KType) -> usize {
    ...
    info!("[DELAY={}, BPS={}, GHOSTDAG K={}]", delay, bps, k);
    info!("[Average stats of generated DAG] blues: {}, reds: {}, parents: {}, txs: {}", blues_mean, reds_mean, parents_mean, txs_mean);
    num_txs
}
