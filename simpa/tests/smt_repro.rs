...
fn import_exported_pruning_point_smt(
    ...
    (result.root, result.lanes_imported)
}
...
impl ChurningLaneProducer {
    ...
    fn lane_number(&self, ctx: &LaneContext) -> u32 {
        ...
        start + (((x >> 32) as u32 + ctx.tx_index as u32) % width) + 1
    }

    fn mix(&self, ctx: &LaneContext) -> u64 {
        let mut x = self.seed ^ ctx.sim_time.rotate_left(7) ^ ctx.block_index.rotate_left(17) ^ ctx.miner_id.rotate_left(41);
        x ^= x >> 33;
